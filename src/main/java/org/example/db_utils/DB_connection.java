...
public class DB_connection {
    ...
    private static PGSimpleDataSource getBaseDataSource(String database){
        PGSimpleDataSource pgSimpleDataSource = new PGSimpleDataSource(){{
            setServerName(ConfigurationManager.getBadeConfig().server());
            setPortNumber(ConfigurationManager.getBadeConfig().port());
            setUser(ConfigurationManager.getBadeConfig().user());
            setPassword("1308");
